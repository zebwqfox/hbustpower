...
**它只被读取，不接收任何数据**；请求不带 Cookie、不带参数、不带设备标识。
...
可用的开关：`campusCard`、`recharge`、`updateCheck`。

## 用起来

- 启动或回到前台时**每天最多查一次**；用户在 设置 → 更新 里手动点则每次都查。
- 带 `ETag` 请求，没改动时服务端返回 304，只有几百字节。给 `config.json` 配上 `ETag` 和
  `Cache-Control: max-age=600` 就够了。
- 响应超过 64 KB 会被拒绝。
